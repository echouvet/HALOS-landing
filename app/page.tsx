...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-halos-paper text-halos-ink">
      ...
      {/* --- HEADER --- */}
      <header className="sticky top-0 z-20 border-b border-white/30 bg-white/30 shadow-[inset_0_-1px_0_0_rgba(47,71,99,0.05),0_8px_24px_-12px_rgba(47,71,99,0.15)] [-webkit-backdrop-filter:url(#liquid-glass)_blur(8px)_saturate(180%)] [backdrop-filter:url(#liquid-glass)_blur(8px)_saturate(180%)]">
        <div className="mx-auto flex h-[72px] max-w-[1280px] items-center justify-between px-5 md:px-10">
          <a href="/" className="flex items-center gap-2.5 text-halos-ink no-underline" aria-label="HALOS Bio home">
          ...
          {/*    className="hidden text-[13px] text-halos-ink transition-opacity hover:opacity-70 sm:inline"*/}
          {/*  >*/}
          {/*    Contact*/}
          {/*  </a>*/}
          {/*  <button*/}
          ...
          {/*    className="rounded-lg bg-halos-ink px-4 py-2.5 text-[13px] font-bold text-white transition-colors hover:bg-halos-ink-800"*/}
          {/*  >*/}
          {/*    Join waitlist*/}
          {/*  </button>*/}
          {/*</nav>*/}
      ...
      {/* --- HERO --- */}
      <section className="relative flex min-h-screen flex-col overflow-hidden border-b border-halos-ink-200">
        ...
        <div aria-hidden="true" className="pointer-events-none absolute inset-0">
          <SparklesCore
            ...
            maxSize={2.4}
            particleDensity={100}
            particleColor={["#6B73C9", "#8A6FC9", "#A87BC2", "#C68BC9"]}
            // particleColor={["#5560B0", "#7458B5", "#8E62A8", "#AE6FB3"]}
            speed={0.6}
            className="h-full w-full"
          />
        </div>

        {/* White fade overlay over sparkles only — fully transparent down to ~the action buttons,
            then easing to ~95% white toward the bottom of the hero. Painted before the blobs
            so the gradient blobs remain crisp on top. */}
        <div
          aria-hidden="true"
        ...
        <div className="relative mx-auto flex w-full max-w-[1280px] flex-1 flex-col px-5 py-20 md:px-10 md:py-24">
          ...
          {/* H1 */}
          <h1 className="m-0 max-w-[1100px] text-balance font-display text-[clamp(44px,6.2vw,96px)] font-light leading-[1.02] tracking-[-0.022em] text-halos-ink">
            We are engineering the most <b className="font-bold">regenerative</b> biochemical envir
            <span
              className="bg-halos-gradient bg-clip-text text-transparent"
              aria-hidden="true"
            >
              o
            </span>
          ...
          {/* Sub */}
          <p className="mb-10 mt-7 max-w-[1000px] text-[20px] font-light leading-[1.45] text-halos-ink-600">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <button
              type="button"
              onClick={() => setDialogOpen(true)}
              className="group inline-flex items-center gap-2 rounded-full border border-white/45 bg-white/5 px-6 py-3 text-[14px] font-bold tracking-[0.01em] text-halos-ink shadow-[inset_0_1px_0_0_rgba(255,255,255,0.55),inset_0_-1px_0_0_rgba(0,0,0,0.08),0_10px_30px_-10px_rgba(47,71,99,0.25)] transition-all duration-200 hover:bg-halos-ink/5 active:scale-[0.98] [-webkit-backdrop-filter:url(#liquid-glass)_blur(4px)_saturate(180%)] [backdrop-filter:url(#liquid-glass)_blur(4px)_saturate(180%)]"
            >
              Join waitlist
            ...
            <a
              href="mailto:eloi@halos.bio"
              className="inline-flex items-center gap-2 rounded-full border border-white/45 bg-halos-ink/10 px-6 py-3 text-[14px] font-bold tracking-[0.01em] text-halos-ink shadow-[inset_0_1px_0_0_rgba(255,255,255,0.55),inset_0_-1px_0_0_rgba(47,71,99,0.04),0_10px_30px_-10px_rgba(47,71,99,0.18)] transition-all duration-200 hover:bg-halos-ink/15 active:scale-[0.98] [-webkit-backdrop-filter:url(#liquid-glass)_blur(4px)_saturate(180%)] [backdrop-filter:url(#liquid-glass)_blur(4px)_saturate(180%)]"
            >
          ...
          <div className="mt-8 flex flex-wrap items-center gap-x-6 gap-y-2 text-[13px] text-halos-ink-500">
            <div className="flex items-center gap-2">
              <FlaskConical className="h-4 w-4 text-halos-ink-400" aria-hidden="true" />
              <span>Research-Focused</span>
            </div>
            <div className="flex items-center gap-2">
              <ShieldCheck className="h-4 w-4 text-halos-ink-400" aria-hidden="true" />
              <span>Ethically Rigorous</span>
            </div>
          </div>

          {/* Compliance / disclaimer — anchored at the bottom of the hero */}
          <p className="mt-auto flex max-w-3xl items-start gap-2.5 pt-16 text-[13px] leading-[1.55] text-halos-ink-500">
            <ShieldCheck className="mt-0.5 h-4 w-4 flex-shrink-0 text-halos-ink-400" aria-hidden="true" />
            <span>
              <strong className="font-bold text-halos-ink">R&amp;D stage.</strong> All information is exploratory and
      ...
      <footer className="bg-white pb-16 pt-12">
        <div className="mx-auto max-w-[1280px] px-5 md:px-10">
          <div className="grid grid-cols-1 gap-10 md:grid-cols-[2fr_1fr_1fr]">
            <div className="flex items-start gap-4">
              ...
              <p className="max-w-[280px] text-[12px] leading-[1.5] text-halos-ink-500">
                HALOS Biosciences, Inc. Pioneering artificial human fetal serum for longevity and regenerative
                medicine.
              </p>
            </div>
            <div>
              <h5 className="mb-3 text-[11px] font-bold uppercase tracking-[0.12em] text-halos-ink-500">Company</h5>
              <ul className="m-0 list-none p-0">
                <li className="py-1">
                  <a href="/privacy" className="text-[13px] text-halos-ink no-underline transition-opacity hover:opacity-70">
                    Privacy policy
                  </a>
                </li>
                {/* <li className="py-1">
            ...
            <div>
              <h5 className="mb-3 text-[11px] font-bold uppercase tracking-[0.12em] text-halos-ink-500">Contact</h5>
              <ul className="m-0 list-none p-0">
                <li className="py-1">
                  <a
                    href="mailto:eloi@halos.bio"
                    className="text-[13px] text-halos-ink no-underline transition-opacity hover:opacity-70"
                  >
                    eloi@halos.bio
                  </a>
                </li>
                <li className="py-1">
                  <button
                    type="button"
                    onClick={() => setDialogOpen(true)}
                    className="bg-transparent p-0 text-[13px] text-halos-ink transition-opacity hover:opacity-70"
                  >
                    Join waitlist
                  </button>
                </li>
          ...
          <div className="mt-10 flex flex-col justify-between gap-2 border-t border-halos-ink-200 pt-5 text-[11px] uppercase tracking-[0.08em] text-halos-ink-500 sm:flex-row">
